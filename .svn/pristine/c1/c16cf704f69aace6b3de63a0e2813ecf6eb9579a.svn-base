...
<template>
  <div class="AuditPlan-container">
    ...
    >
      <div class="left">
        <div style="width:100%;height:40px;background:#4C4E52">
          ...
          <el-button type="text" @click="finish">初审完成</el-button>
        </div>
    ...
    <el-dialog title="新增意见" :visible.sync="dialogView" width="400px">
      <el-form ref="formInline" label-width="100px" class="marginBottom">
    ...
    <el-dialog :visible.sync="dialogtip" width="500px">
      <el-form ref="form" class="el-form" style="line-height:75px"  label-width="80px">
        <div style="text-align:center;width:100%;float:left">
          <span style="text-align:center;width:100%;float:left;line-height:40px">你确定初审完成？</span>
          <span
            style="text-align:center;width:100%;float:left;line-height:40px"
          >确定后，图纸将进入复核阶段，初审意见不可再修改！</span>
        </div>
        <div style="text-align:center;">
          ...
          <span style="text-align:right;width:100%;">
            <el-button size="small" style="margin-left:20px;" @click="dialogtip = !dialogtip">取消</el-button>
          </span>
        </div>
      </el-form>
    </el-dialog>
...
import { checkOpinion } from "../../api/checkOpinion";
export default {
  data() {
    return {
      list: [
      ...
      version: "1",
      reviewer: "审核人",
      id: "0ebb56683f2d4461a7213bb6d3c67a81",
      checker: "复查人",
      result: "带复查"
    };
  },
  ...
  methods: {
    ...
    dialogtipShow() {
      ...
      sessionStorage.setItem("userAdvise", JSON.stringify(this.newData));
    ...
    sure() {
      let advise = {};
      advise.opinion = this.advise;
      advise.opinionType = this.opinionType;
      advise.createTime = this.timer;
      advise.updateTime = this.timer;
      advise.pageSize = this.pageSize;
      advise.state = this.state;
      advise.version = this.version;
      advise.reviewer = this.reviewer;
      advise.id = this.id;
      advise.ecode = "884354399677456";
      // advise.ecode = this.ecode;
      advise.checker = this.checker;
      advise.result = this.result;
      //新增意见接口
      checkOpinion(advise)
        .then(result => {
          console.log(result);
