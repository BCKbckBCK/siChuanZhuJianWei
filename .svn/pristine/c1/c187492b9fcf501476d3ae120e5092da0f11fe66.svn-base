  ...
  <div class="processDetail-container">
    <div class="detail">
      ...
      <el-table
        ...
        style="margin-top:20px;line-height:48px;font-size:16px"
      >
        ...
        <el-table-column label="操作" :show-overflow-tooltip="true" width="100">
          <template slot-scope="scope">
            <el-button v-if="scope.row.td6===1" type="text" @click="deleteConfirm(scope.$index)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { addPersonInfo, deletePersonInfo } from "../../api/personInfo.js";
export default {
  name: "Process",
  data() {
    return {
  ...
  methods: {
    addNewPerson() {
      let arrLength = this.list.length - 1;
      const newPerson = {
        td0: "主要参与人员",
        td1: "",
        td2: "",
      ...
      this.list.splice(arrLength, 0, newPerson);
    },
    fetchData() {
      // const that = this
    ...
    deleteConfirm(index) {
      this.$confirm("删除此工程 是否继续?", "提示", {
        ...
        this.deleteUser(index).then(res => {
          this.$message({
            message: "删除成功",
            type: "success"
          });
