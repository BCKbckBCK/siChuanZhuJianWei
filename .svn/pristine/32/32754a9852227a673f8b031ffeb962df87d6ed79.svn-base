...
export default {
    ...
    mounted () {
        ...
        if (this.$route.query.id) {
            ...
            const _tmp1 = this.list.find(x => x.td00.toString() === this.$route.query.id.toString())
            if (_tmp) {
                _tmp.td6 = 2
            }
            if (_tmp1) {
    ...
    methods: {
        ...
        appoint (item) {
            this.$router.push({ path: 'taskAppoint', query: { id: item.td00 }})
        },
        checkResult(){
            this.$router.push({ path: 'checkResult',query: { id: 1 }})
        }
    }
}
