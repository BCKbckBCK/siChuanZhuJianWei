    ...
    <div class="detail">
      ...
      >
        ...
        <el-table-column label="操作" :show-overflow-tooltip="true" width="100">
          <template slot-scope="scope">
            <el-button style="font-size:16px" v-if="scope.row.td6==1" type="text" @click="view(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <span style="text-align:right;width:100%;display:block;margin-top:10px">
        <el-button style="font-size:16px;margin-left:2px" size="small" @click="goBack()">
          返回<i class="el-icon-arrow-right el-icon--right" style="font-size:16px;margin-left:-3px"/>
        </el-button>
      </span>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    view (item) {
      ...
      this.list = this.list.splice(item.index,1)
    },
    deleteConfirm (item) {
