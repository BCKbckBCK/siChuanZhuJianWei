        ...
        <el-table-column label="工作量" :show-overflow-tooltip="true" style="width:20%">
          <template slot-scope="scope">
            <input
              ...
              v-model="scope.row.td4"
            />
          </template>
        </el-table-column>
        <el-table-column label="备注" style="width:20%">
          <template slot-scope="scope">
            <input class="workRecordInput" v-model="scope.row.td5" style="padding:0 10px" />
          </template>
        </el-table-column>
      </el-table>
      <span style="text-align:right;width:100%;display:block">
        <div style="padding-top:10px">
          <el-button size="small" class="workRecordSave" style="margin-bottom:10px" @click="goBack">
            保存
...
export default {
  ...
  methods: {
    getWorkRecord() {
      ...
      this.workRecordId = this.$route.query.id;
    },
    ...
    goBack() {
      this.$router.replace({ path: "/dispatch/rejectProj",query: { status: this.workRecordStatus,id:this.workRecordId } });
